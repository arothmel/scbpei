...
-->
   <xsl:template name="rewritehrefs" match="node()[@href]">
	...
	-->
      ...
      <xsl:with-param name="replace_suffix"><xsl:value-of select="$replace_suffix" /></xsl:with-param>
			<!--
...
<!-- 
 ...
 -->
   <xsl:template name="rewritesrcs" match="node()[@src]">
      ...
      <xsl:with-param name="replace_suffix"></xsl:with-param>
		</xsl:call-template>
   </xsl:template>
...
<!-- 
 ...
 -->  
   <xsl:template name="rewriteactions" match="node()[@action]">
    <xsl:call-template name="rewritelink">
      <xsl:with-param name="attname">action</xsl:with-param>
      <xsl:with-param name="linkbase"><xsl:value-of select="$href_base" /></xsl:with-param>
      <xsl:with-param name="linkroot"><xsl:value-of select="$site_root" /></xsl:with-param>
      <xsl:with-param name="replace_suffix"></xsl:with-param>
    </xsl:call-template>
   </xsl:template>
	  ...
	  <xsl:copy>
		 <xsl:for-each select="@*" >
			<xsl:choose>
              ...
              <xsl:call-template name="replace_suffix"><xsl:with-param name="path"><xsl:value-of select="." /></xsl:with-param><xsl:with-param name="replace_suffix"><xsl:value-of select="$replace_suffix" /></xsl:with-param></xsl:call-template>
							</xsl:attribute>
						</xsl:when>
            ...
            <xsl:when test="starts-with(., '/')" >
              ...
              <xsl:attribute name="{$attname}">
               ...
               <xsl:call-template name="replace_suffix"><xsl:with-param name="path"><xsl:value-of select="substring( . ,2)" /></xsl:with-param><xsl:with-param name="replace_suffix"><xsl:value-of select="$replace_suffix" /></xsl:with-param></xsl:call-template>
              </xsl:attribute>
            </xsl:when>
     ...
     <xsl:param name="replace_suffix" />
      <xsl:choose>
        ...
        <xsl:when test="contains($path, concat('/', $default_document))">
          ...
          -->
          <xsl:value-of select="substring-before($path, concat('/', $default_document))" />
        </xsl:when>
